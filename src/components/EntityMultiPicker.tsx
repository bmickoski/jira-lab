...
export function EntityMultiPicker<T extends EntityBase>({
  ...
  // Build a unified options model: [create?] + items
  ...
  <div ref={containerRef} className="w-full">
      ...
      >
        ...
        <button
          ...
          className={[
            ...
            disabled || value.length === 0 ? "cursor-not-allowed opacity-60 hover:bg-transparent" : "cursor-pointer",
          ].join(" ")}
        >
          Clear
        </button>
      ...
      {picker.open && (
        <div className="mt-2 overflow-hidden rounded-xl border border-white/10 bg-neutral-950/60 backdrop-blur">
          <div className="border-b border-white/10 px-3 py-2 text-xs text-white/70">{picker.statusText}</div>

          {/* NON-VIRTUAL LIST */}
          {!virtualize && (
            <div id={listboxId} role="listbox" aria-labelledby={labelId} className="max-h-60 overflow-auto">
              {picker.hasCreateRow && (
                <div
                  role="option"
                  aria-selected={picker.activeIndex === 0}
                  aria-disabled={picker.loading}
                  onMouseEnter={() => picker.setActiveIndex(0)}
                  onMouseDown={(e) => {
                    e.preventDefault();
                    if (!picker.loading) picker.createAndAdd();
                  }}
                  className={[
                    "border-b border-white/10 px-3 py-2 font-semibold",
                    picker.activeIndex === 0 ? "bg-white/10" : "",
                    picker.loading ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-white/10",
                  ].join(" ")}
                >
              ...
              {picker.visibleItems.map((it, idx) => {
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => picker.setActiveIndex(uiIndex)}
                    onMouseDown={(e) => {
                      e.preventDefault();
                      picker.add(it);
                    }}
                    className={[
                      ...
                      picker.canSelectMore ? "cursor-pointer hover:bg-white/10" : "cursor-not-allowed opacity-60",
                    ].join(" ")}
                  >
                    {renderItem ? (
                ...
                {rowVirtualizer.getVirtualItems().map((vRow) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={() => picker.setActiveIndex(vRow.index)}
                      onMouseDown={(e) => {
                        e.preventDefault();
                        if (!opt) return;
                    ...
                    >
                      {!opt ? null : opt.kind === "create" ? (
                        ...
                        <>
                          ...
                          {opt.item.subLabel && <div className="text-xs text-white/60">{opt.item.subLabel}</div>}
                        </>
                      )}
                    </div>
                  );
